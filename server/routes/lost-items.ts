...
router.get('/:id', verifyToken, async (req: Request, res: Response) => {
    let db = database.getDb();
    let lostItemData = await db.collection("lostItem").findOne({ _id: new ObjectId(req.params.id as string)});
...
router.put('/:id', verifyToken, async (req: Request, res: Response) => {
    ...
    let lostItemData = await db.collection("lostItem").updateOne({ _id: new ObjectId(req.params.id as string) }, newItem);
    res.json(lostItemData);
});
...
router.delete('/:id', verifyToken, async (req: Request, res: Response) => {
    let db = database.getDb();
    let lostItemData = await db.collection("lostItem").deleteOne({ _id: new ObjectId(req.params.id as string)});
